...
struct Node{
	...
	// "int *pages" � ponteiro para o primeiro id do array
	int *pages;
	int size;
	Node *pChild[37];
...
class Trie{
	...
	void search_keys(string *keys, int size, int *&res, int &len_r){
		...
		// "size" � numero de palavras na requisicao
		...
		for(int i = 1; i < size; i++){
			// interseccionamos entre listas a cada la�o
	...
	void search(string key, int &len, int *&res){
		...
		// buscamos iteradamente at� o ultimo node
		Node *pNode = pRoot;
		for(char c: key){
	...
	vector<string> suggestion(string key, int maxCost){
		...
		//chamo a fun��o recursiva para cada um dos filhos
	...
	void recursive_sug(Node *pNode, int cur_letter, int key_ar[], int key_sz, int previousRow[],vector<string>& result_words, int cur_word[], int pos, int maxCost){
		...
		//se o �ltimo elemento da linha do Leveshstein for menor que maxCost
		...
		//se o custo minimo da coluna for menor que o custo m�ximo
...
string get_page(string page, int *res){
	// carregamos a p�gina do artigo desejado
	string text = "";
	string aux = "";
	ifstream file;
